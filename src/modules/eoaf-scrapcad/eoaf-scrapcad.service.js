...
class EoafScrapcadService {
  ...
  async getFormById(id) {
    const { rows } = await eoafQuery(
      'SELECT TOP 1 * FROM xoaf_scrapcad_form_s WHERE r_object_id = @p1',
      [id]
    );
    return rows[0] || null;
  }
  ...
  async listEnclosures({ page = 1, limit = 50, form_id, related_memo_id }) {
    const offset = (parseInteger(page) - 1) * parseInteger(limit);
    ...
    const queryParams = [...params, parseInteger(limit), offset];
    const { rows } = await eoafQuery(
      `SELECT * FROM xoaf_scrap_enclosures_s ${whereClause} ORDER BY r_object_id ASC OFFSET @p${params.length + 1} ROWS FETCH NEXT @p${params.length + 2} ROWS ONLY`,
      queryParams
    );
  ...
  async getEnclosureById(id) {
    const { rows } = await eoafQuery(
      'SELECT TOP 1 * FROM xoaf_scrap_enclosures_s WHERE r_object_id = @p1',
      [id]
    );
    return rows[0] || null;
  }
...
module.exports = new EoafScrapcadService();
